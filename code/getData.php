...
$queryAll = $db->query("SELECT COUNT(*) as num_rows FROM details WHERE id < ".$lastID." ORDER BY id DESC");
$rowAll = $queryAll->fetch_assoc();
$allNumRows = $rowAll['num_rows'];

//Get rows by limit except already displayed
$query = $db->query("SELECT * FROM details WHERE id < ".$lastID." ORDER BY id DESC LIMIT ".$showLimit);
